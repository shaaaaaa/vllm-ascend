...
def diagnose_request(llm, args, ordinal: int) -> dict:
    ...
    for worker in workers:
        ...
        if args.child == "full" and stages.get("signature.validate", {}).get("wall", {}).get("count", 0):
            raise RuntimeError(f"Unexpected hot-path input validation on rank {worker['rank']}")
