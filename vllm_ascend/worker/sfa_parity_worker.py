...
from vllm_ascend.worker.sfa_fixture import (
    HASH_CHUNK_BYTES as HASH_CHUNK_BYTES,
)
from vllm_ascend.worker.sfa_fixture import (
    TARGET_LAYERS,
    deterministic_dummy_load,
    prepare_dummy_quant_config,
)
from vllm_ascend.worker.sfa_fixture import (
    remap_mtp_quant_description as remap_mtp_quant_description,
)
...
class LayerSnapshots:
    ...
    def install_sfa_probes(self, impl: AscendSFAImpl) -> None:
        topk_method = (
            "_get_indexcache_topk_indices" if getattr(impl, "skip_topk", False) else "indexer_select_post_process"
        )
        indexer = getattr(impl, topk_method)
...
class SFAParityWorker(NPUWorker):
    ...
    def _prepare_quant_config(self) -> None:
        prepare_dummy_quant_config(self.vllm_config)
