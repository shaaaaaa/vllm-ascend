...
def _cpu_snapshot(value: Any, active_ids: set[int]) -> Any:
    ...
    if is_dataclass(value) and not isinstance(value, type):
        ...
        try:
            payload = {field.name: _cpu_snapshot(getattr(value, field.name), active_ids) for field in fields(value)}
        finally:
    ...
    if isinstance(value, Mapping):
        active_ids.add(value_id)
        try:
            return {str(key): _cpu_snapshot(item, active_ids) for key, item in value.items()}
        finally:
            active_ids.remove(value_id)
    ...
    if isinstance(value, tuple):
        active_ids.add(value_id)
        try:
            return {"__tuple__": [_cpu_snapshot(item, active_ids) for item in value]}
        finally:
            active_ids.remove(value_id)
...
def snapshot_cache_components(
    ...
    *,
    ...
    """Copy only cache blocks referenced by the current draft invocation."""
    ...
    for component_index, component in enumerate(cache):
        if not isinstance(component, torch.Tensor):
            ...
            continue
        block_ids = indexer_block_ids if component_index >= 2 else latent_block_ids
...
def _snapshot_cache_tensor(
    ...
    valid_ids = [int(block_id) for block_id in block_ids if 0 <= int(block_id) < tensor.shape[0]]
    invalid_ids = [int(block_id) for block_id in block_ids if int(block_id) < 0 or int(block_id) >= tensor.shape[0]]

    # Free-paged latent caches are small dummy tensors. Preserve them whole
    # instead of pretending request block ids address their first dimension.
    ...
    if valid_ids:
        indices = torch.tensor(valid_ids, dtype=torch.long, device=tensor.device)
        blocks = tensor.index_select(0, indices).detach().cpu().clone()
    else:
        blocks = torch.empty((0, *tensor.shape[1:]), dtype=tensor.dtype, device="cpu")
    return {
        "layout": layout,
        "whole": False,
