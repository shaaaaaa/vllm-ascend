...
class AscendSFAMetadata:
    ...
    # HW-VERIFY: confirm the source 閳?req_ids/prompt_lens live on the runner's
...
class AscendSFAMetadataBuilder(MLACommonMetadataBuilder[AscendSFAMetadata]):
    ...
    def build(
        ...
        # for scratch_remap 閳?decode rows (position >= prompt_len) get plen,
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def forward(
        ...
        if self.dsa_offload_unbundle and len(kv_cache) < 3:
            ...
            # the indexer read/write (kv_cache[2]) work unchanged 閳?both groups share the
        ...
        # native
            ...
            # (this one with a dense arange) and desync it 閳?skip whenever the
        ...
        # All fixed-shape device math 閳?no D2H sync. No-op without a connector.
        ...
        if (
            ...
            # length, prefill/padding rows carry 0 and stay untouched 閳?so this
        ...
        if attn_output is None and _dsa_supported:
            ...
            # (is_output_kv=True), aligned with token order 閳?no paged read-back, so this
