...
def test_dense_prefill_skips_cold_resume_probe(native_api):
    runner, metadata, kwargs = native_case(
        native_api,
        [4],
        [0],
    ...
    def unexpected(*args, **kwargs):
        raise AssertionError("dense prefill entered the cold-resume probe")

    native_api["np"] = NS(asarray=unexpected)
    native_api["unwrap_staged_sfa_connector_metadata"] = unexpected
    decision = native_api["_staged_sfa_local_route"](runner, **kwargs)
    assert decision.reason.value == "not_configured"
    assert not decision.cold_compact_resumes


def test_prefill_bank_rebinding_preserves_cold_proof_and_rejects_sparse_reuse(native_api):
    ...
    method = next(
        ...
        if isinstance(n, ast.FunctionDef) and n.name == "_layerwise_prefill_common_attn_metadata"
    )
    future = ast.ImportFrom(module="__future__", names=[ast.alias(name="annotations")], level=0)
    ns = dict(copy=copy)
    exec(
        compile(ast.fix_missing_locations(ast.Module(body=[future, method], type_ignores=[])), "bank_rebind", "exec"),
        ns,
    )
    runner, metadata, kwargs = native_case(
        native_api,
    ...
    metadata.cold_compact_resumes = decision.cold_compact_resumes
    banks = {
        (group, bank): (torch.full((2, 16), 10 * group + bank), torch.arange(6) + 100 * group + 10 * bank)
        for group in (0, 1)
        for bank in (0, 1)
    }
    rebound = ns["_layerwise_prefill_common_attn_metadata"](
        runner,
        metadata,
        0,
        1,
        0,
        lambda group, bank: banks[group, bank],
    )
    assert rebound is not metadata
    assert rebound.cold_compact_resumes is metadata.cold_compact_resumes
    assert rebound.cold_compact_resumes.computed_ends == (22683, 0)
    sfa_builder = builder.__wrapped__(native_api)
    result = sfa_builder.build(0, rebound)
    assert result.block_table.data_ptr() == banks[0, 1][0].data_ptr()
    assert result.indexer_block_table.data_ptr() == banks[1, 0][0].data_ptr()
    assert result.decode_req_indices.tolist() == [0, 0, -1, -1, -1, -1]
    assert sfa_builder.rebind_layerwise_prefill_metadata(result, metadata) is None


@pytest.mark.parametrize("captures", [(4, 8, 16, 32), ()])
def test_aligned_checkpoint_frontier_is_not_the_computed_end(native_api, captures):
    runner, metadata, kwargs = native_case(
        native_api, [2, 1], [22683, 22684], [22684, 22684], [22684, 22685],
        np.array([1, 0]), {0, 1}, captures,
    )
    for i, request in enumerate(kwargs["kv_connector_metadata"].requests):
        request.load_spec.lmcache_cached_tokens = 22684
        request.load_spec.dsa_committed_end = 22684
        request.load_spec.dsa_cold_computed_end = 22683 + i
        request.load_spec.dsa_remap_frontier = 22528
        request.dsa_nonresident_frontier = 22528
    decision = native_api["_staged_sfa_local_route"](runner, **kwargs)
    metadata.cold_compact_resumes = decision.cold_compact_resumes
    result = builder.__wrapped__(native_api).build(0, metadata)
    assert decision.frontiers == (22528, 22528)
    assert decision.cold_compact_resumes.computed_ends == (22683, 22684)
    assert result.decode_req_indices.tolist() == [0, 0, 1]
    boundary = native_api["_update_dsa_split_boundary_in_place"](result, [22528, 22528], 0)
    assert boundary.tolist() == [22528, 22528, 22528]


def test_real_aligned_lookup_retains_graph_eligible_computed_end(native_api):
    import importlib.util
    import sys

    path = Path(__file__).resolve().parents[3] / "LMCache-NPU/tests/standalone/test_sparse_release_frontier.py"
    spec = importlib.util.spec_from_file_location("aligned_lookup_contract", path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    callbacks = module.callbacks.__wrapped__()
    adapter, _, _ = module.make_request(callbacks, prompt=22684)
    adapter.config.decode_preemption_checkpoint = True
    adapter._lmcache_chunk_size = 1024
    computed = adapter.get_num_new_matched_tokens(NS(request_id="repro", num_tokens=22684), 0)
    load = adapter.load_specs["repro"]
    assert computed == 22683 and load.dsa_remap_frontier == 22528
    load.can_load = True
    load.dsa_cold_compact_resume = True
    runner, metadata, kwargs = native_case(
        native_api, [2], [computed], [22684], [22684], np.array([1]), {0},
    )
    request = kwargs["kv_connector_metadata"].requests[0]
    request.load_spec = load
    request.dsa_nonresident_frontier = load.dsa_remap_frontier
    decision = native_api["_staged_sfa_local_route"](runner, **kwargs)
    assert decision.action.value == "staged"
    assert decision.frontiers == (22528,)
    assert decision.cold_compact_resumes.computed_ends == (computed,)


@pytest.mark.parametrize("end", [22527, 22685])
def test_aligned_computed_end_must_stay_within_loaded_cache(native_api, end):
    _, metadata, kwargs = native_case(
        native_api, [2], [22683], [22684], [22684], np.array([1]), {0},
    )
    request = kwargs["kv_connector_metadata"].requests[0]
    request.dsa_nonresident_frontier = 22528
    request.load_spec.dsa_remap_frontier = 22528
    request.load_spec.dsa_cold_computed_end = end
    reason, frontiers, markers = native_api["staged_sfa_metadata_sparse_route"](
        kwargs["kv_connector_metadata"], metadata.request_ids,
    )
    assert reason.value == "invalid_frontier"
    assert not frontiers and not markers
