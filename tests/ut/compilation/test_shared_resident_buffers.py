...
def test_ascend_aot_keeps_group_storage_without_clones(planner_api, mtp, v2):
    _, (producer, consumer), group, _ = grouped(planner_api, mtp)
    ns = dict(
        torch=torch,
    ...
    namespace = f"resident_islands_{mtp}_{int(v2)}"
