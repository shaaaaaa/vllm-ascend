...
PREFILL_TIMING_DEBUG="${PREFILL_TIMING_DEBUG:-false}"
PORT="${PORT:-9960}"
...
printf 'MODE=%s\nLAYERWISE=%s\nMAX_MODEL_LEN=%s\nPREFILL_PROFILE_EDGE_CHUNKS=%s\nPREFILL_TIMING_DEBUG=%s\nLOG=%s\nPROFILE_DIR=%s\n' \
  "${MODE}" "${LAYERWISE}" "${MAX_MODEL_LEN}" \
  "${PREFILL_PROFILE_EDGE_CHUNKS}" "${PREFILL_TIMING_DEBUG}" \
  "${LOG}" "${PROFILE_DIR}"
...
VLLM_ASCEND_PREFILL_TIMING_DEBUG="${PREFILL_TIMING_DEBUG}" \
