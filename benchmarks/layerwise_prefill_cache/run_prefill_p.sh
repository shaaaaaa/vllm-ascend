...
PREFILL_PROFILE_LAST_CHUNKS="${PREFILL_PROFILE_LAST_CHUNKS:-0}"
PORT="${PORT:-9960}"
...
printf 'MODE=%s\nLAYERWISE=%s\nMAX_MODEL_LEN=%s\nPREFILL_PROFILE_LAST_CHUNKS=%s\nLOG=%s\nPROFILE_DIR=%s\n' \
  "${MODE}" "${LAYERWISE}" "${MAX_MODEL_LEN}" \
  "${PREFILL_PROFILE_LAST_CHUNKS}" "${LOG}" "${PROFILE_DIR}"

VLLM_USE_V1=1 \
VLLM_ALLOW_LONG_MAX_MODEL_LEN="${ALLOW_LONG_MAX_MODEL_LEN}" \
VLLM_ASCEND_LAYERWISE_PREFILL_P_NODE="${LAYERWISE}" \
VLLM_ASCEND_PREFILL_PROFILE_LAST_CHUNKS="${PREFILL_PROFILE_LAST_CHUNKS}" \
