...
#include "common.h"
namespace redesign {
// Each block owns one Slice-sized range. All writes stay inside that private
// aligned slice, including collision handling. No atomics or spin barriers.
template<uint32_t Slice = T> class BuildTable {
    AscendC::TPipe pipe;
...
public:
    __aicore__ inline void Init(RedesignLaunch args) {
        ...
        pipe.InitBuffer(buf, 7 * n * 4 + 2 * (n / 8) + Slice * 4);
    }
    __aicore__ inline void Process() {
        uint32_t n = a.queries * a.topk, slices = (a.tableSize + Slice - 1) / Slice;
        ...
        for (uint32_t block = AscendC::GetBlockIdx(); block < a.requests * slices;
            ...
            uint32_t request = block / slices, base = (block % slices) * Slice;
            uint32_t countCells = a.tableSize - base < Slice ? a.tableSize - base : Slice;
            ...
            Range(mask, bucket, tmp, base, base + countCells, n);
            ...
            AscendC::Duplicate(out, static_cast<int32_t>(-1), countCells); V();
            ...
            AscendC::DataCopy(table[static_cast<uint64_t>(request) * a.tableSize + base], out, countCells);
