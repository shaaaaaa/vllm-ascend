...
// Experimental v2 planner. Each request is split into one compact-output
// role, one state-update role per shard, and one remap role per position
// partition. Every role reads immutable union output and owns disjoint GM
// cachelines, so the plan needs neither a request-wide finalize AIV nor
// inter-AIV synchronization.
class DSAResidentParallelPlanV2Kernel {
public:
    struct Metadata {
        uint32_t currentCounts[kMaxResidentShards] = {};
        uint32_t missCounts[kMaxResidentShards] = {};
        uint32_t evictCounts[kMaxResidentShards] = {};
        uint32_t oldCounts[kMaxResidentShards] = {};
        uint32_t missPrefixes[kMaxResidentShards] = {};
        uint32_t evictPrefixes[kMaxResidentShards] = {};
        uint32_t selectedCounts[kMaxResidentShards] = {};
        uint32_t poolOffsets[kMaxResidentShards] = {};
        uint32_t totalMiss = 0;
        uint32_t totalEvict = 0;
        ...
        uint32_t blockSize)
    {
        ...
        blockSize_ = blockSize;
        partWidth_ = requestWidth_ / shardCount_;
        rolesPerRequest_ = 1 + 2 * shardCount_;
        blockTableEntries_ =
            (capacity_ + blockSize_ - 1) / blockSize_;
        ...
        pipe_.InitBuffer(i32ABuf_, capacity_ * sizeof(int32_t));
        pipe_.InitBuffer(i32BBuf_, capacity_ * sizeof(int32_t));
        pipe_.InitBuffer(i32CBuf_, capacity_ * sizeof(int32_t));
        pipe_.InitBuffer(i32DBuf_, capacity_ * sizeof(int32_t));
        pipe_.InitBuffer(i16ABuf_, capacity_ * sizeof(int16_t));
        pipe_.InitBuffer(i16BBuf_, capacity_ * sizeof(int16_t));
        pipe_.InitBuffer(i16CBuf_, capacity_ * sizeof(int16_t));
        const uint32_t poolElements =
            capacity_ + shardCount_ * kInt16PerDataBlock;
        pipe_.InitBuffer(
            evictPoolBuf_, poolElements * sizeof(int16_t));
        pipe_.InitBuffer(i64Buf_, capacity_ * sizeof(int64_t));
        pipe_.InitBuffer(maskABuf_, capacity_ / 8);
        pipe_.InitBuffer(maskBBuf_, capacity_ / 8);
        const uint32_t blockTableBytes =
            (blockTableEntries_ * sizeof(int32_t)
                + kDataBlockBytes - 1)
            & ~(kDataBlockBytes - 1);
    ...
    {
        const uint32_t block = AscendC::GetBlockIdx();
        const uint32_t request = block / rolesPerRequest_;
        const uint32_t role = block % rolesPerRequest_;
        if (request >= requestCount_) {
            return;
        }
        Metadata meta;
        LoadMetadata(request, meta);
        if (role == 0) {
            ProcessOutput(request, meta);
        } else if (role <= shardCount_) {
            ProcessState(request, role - 1, meta);
        } else {
            ProcessRemap(request, role - 1 - shardCount_, meta);
        }
    }
...
private:
        ...
        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const uint64_t offset =
                static_cast<uint64_t>(request)
                    * shardCountRequestStride_
                + shard * shardCountStride_;
            meta.currentCounts[shard] = static_cast<uint32_t>(
                shardCounts_.GetValue(
                    offset + kShardCurrentCount));
            meta.missCounts[shard] = static_cast<uint32_t>(
                shardCounts_.GetValue(
                    offset + kShardMissCount));
            meta.evictCounts[shard] = static_cast<uint32_t>(
                shardCounts_.GetValue(
                    offset + kShardEvictableCount));
            meta.oldCounts[shard] = static_cast<uint32_t>(
                shardCounts_.GetValue(
                    offset + kShardOldCount));
            meta.missPrefixes[shard] = meta.totalMiss;
            meta.evictPrefixes[shard] = meta.totalEvict;
            meta.totalMiss += meta.missCounts[shard];
            meta.totalEvict += meta.evictCounts[shard];
            meta.totalOld += meta.oldCounts[shard];
        }
        meta.totalSelected = meta.totalMiss < meta.totalEvict
            ? meta.totalMiss
            : meta.totalEvict;
        uint32_t remaining = meta.totalSelected;
        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const uint32_t selected =
                meta.evictCounts[shard] < remaining
                ? meta.evictCounts[shard]
                : remaining;
            const uint32_t aligned =
                (poolEnd + kInt16PerDataBlock - 1)
                & ~(kInt16PerDataBlock - 1);
            meta.selectedCounts[shard] = selected;
            meta.poolOffsets[shard] = aligned;
            poolEnd = aligned + selected;
            remaining -= selected;
        }
    }

    __aicore__ inline void BuildEvictPool(
        uint32_t request,
        const Metadata& meta,
        AscendC::LocalTensor<int16_t> pool)
    {
        ...
        const uint64_t requestShardBase =
            static_cast<uint64_t>(request) * shardCount_ * capacity_;
        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const uint32_t selected = meta.selectedCounts[shard];
            if (selected == 0) {
                continue;
            }
            CopyGlobalToLocalExact(
                pool[meta.poolOffsets[shard]],
                shardEvictableSlots_[
                    requestShardBase
                    + static_cast<uint64_t>(shard) * capacity_],
                selected);
        }
    ...
    __aicore__ inline int16_t ResolveSlotScalar(
        uint32_t globalMiss,
        const Metadata& meta,
        AscendC::LocalTensor<int16_t> pool)
    {
        if (globalMiss < meta.totalSelected) {
            for (uint32_t shard = 0; shard < shardCount_; ++shard) {
                const uint32_t begin = meta.evictPrefixes[shard];
                const uint32_t end = begin + meta.selectedCounts[shard];
                if (globalMiss >= begin && globalMiss < end) {
                    return pool.GetValue(
                        meta.poolOffsets[shard]
                        + globalMiss - begin);
                }
            }
        }
        return static_cast<int16_t>(
            meta.totalOld + globalMiss - meta.totalSelected);
    }

    __aicore__ inline void ResolveSlotsVector(
        const Metadata& meta,
        AscendC::LocalTensor<int32_t> globalMiss,
        AscendC::LocalTensor<int32_t> output,
        AscendC::LocalTensor<int32_t> work,
        AscendC::LocalTensor<int16_t> gathered,
        AscendC::LocalTensor<int16_t> pool,
        ...
        uint32_t count)
    {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const int32_t prefix = static_cast<int32_t>(
                meta.evictPrefixes[shard]);
            AscendC::Maxs(output, globalMiss, prefix, count);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Compare(
                mask,
                output,
            ...
            AscendC::Adds(
                output,
                globalMiss,
                static_cast<int32_t>(meta.poolOffsets[shard])
                    - prefix,
                count);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Select(
                work.ReinterpretCast<float>(),
                mask,
        ...
        AscendC::Gather(
            gathered,
            pool,
            output.ReinterpretCast<uint32_t>(),
            static_cast<uint32_t>(0),
            count);
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Cast(
        ...
        AscendC::Mins(
            output,
            globalMiss,
            static_cast<int32_t>(meta.totalSelected) - 1,
            count);
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Compare(
            mask,
        ...
        AscendC::Adds(
            ...
            globalMiss,
            static_cast<int32_t>(
                meta.totalOld - meta.totalSelected),
            count);
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Select(
            output.ReinterpretCast<float>(),
        ...
        const Metadata& meta)
    {
        ...
        auto work = i32DBuf_.Get<int32_t>();
        auto gathered = i16CBuf_.Get<int16_t>();
        auto pool = evictPoolBuf_.Get<int16_t>();
        auto target64 = i64Buf_.Get<int64_t>();
        auto mask = maskABuf_.Get<uint8_t>();
        auto blockTable = blockTableBuf_.Get<int32_t>();
        ...
        const uint64_t requestOffset =
            static_cast<uint64_t>(request) * capacity_;
        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const uint32_t count = meta.missCounts[shard];
            if (count == 0) {
                continue;
            }
            const uint32_t missBase = meta.missPrefixes[shard];
            CopyGlobalToLocalExact(
                missToken,
                shardMissTokens_[
                    requestShardBase
            ...
            AscendC::CreateVecIndex(
                globalMiss,
                static_cast<int32_t>(missBase),
                count);
            AscendC::PipeBarrier<PIPE_V>();
            ResolveSlotsVector(
                meta, globalMiss, assignedSlot, work,
                gathered, pool, mask, count);
        ...
        missCounts_.SetValue(
            static_cast<uint64_t>(request) * missCountStride_,
            static_cast<int32_t>(meta.totalMiss));
    }

    __aicore__ inline void ProcessState(
        uint32_t request,
        uint32_t shard,
        const Metadata& meta)
    {
        auto oldTokens = i32ABuf_.Get<int32_t>();
        auto currentTokens = i32BBuf_.Get<int32_t>();
        auto mergedTokens = i32CBuf_.Get<int32_t>();
        auto oldSlots = i16ABuf_.Get<int16_t>();
        auto mergedSlots = i16BBuf_.Get<int16_t>();
        auto pool = evictPoolBuf_.Get<int16_t>();
        ...
        while (oldIndex < oldCount || currentIndex < currentCount) {
            ...
            if (haveOld &&
                (!haveCurrent || oldToken < currentToken)) {
                if (evictableIndex >= meta.selectedCounts[shard]) {
                    mergedTokens.SetValue(mergedCount, oldToken);
                    mergedSlots.SetValue(
                        mergedCount, oldSlots.GetValue(oldIndex));
                    ++mergedCount;
                ...
                const uint32_t globalMiss =
                    meta.missPrefixes[shard] + localMiss;
                mergedTokens.SetValue(mergedCount, currentToken);
                mergedSlots.SetValue(
                    mergedCount,
                    ResolveSlotScalar(globalMiss, meta, pool));
                ++localMiss;
                ++currentIndex;
                ++mergedCount;
            } else {
    ...
    __aicore__ inline void ProcessRemap(
        uint32_t request,
        uint32_t part,
        const Metadata& meta)
    {
        ...
        const uint64_t requestShardBase =
            static_cast<uint64_t>(request) * shardCount_ * capacity_;
        auto input = i32ABuf_.Get<int32_t>();
        auto accumulated = i32BBuf_.Get<int32_t>();
        auto rankOrResolved = i32CBuf_.Get<int32_t>();
        auto encodedPrior = i32DBuf_.Get<int32_t>();
        auto scratch = i64Buf_.Get<int32_t>();
        auto work = scratch[capacity_];
        auto mapping = i16ABuf_.Get<int16_t>();
        auto shardPrior = i16BBuf_.Get<int16_t>();
        auto gatheredPrior = i16CBuf_.Get<int16_t>();
        auto pool = evictPoolBuf_.Get<int16_t>();
        auto workFloat = scratch.ReinterpretCast<float>();
        auto selectedMask = maskABuf_.Get<uint8_t>();
        auto hitMask = maskBBuf_.Get<uint8_t>();
        ...
        Sync<AscendC::HardEvent::MTE2_V>();
        AscendC::Duplicate(
            accumulated, static_cast<int32_t>(-1), partWidth_);
        AscendC::PipeBarrier<PIPE_V>();

        for (uint32_t shard = 0; shard < shardCount_; ++shard) {
            const uint32_t count = meta.currentCounts[shard];
            if (count == 0) {
                continue;
            }
            CopyGlobalToLocalExact(
            ...
            AscendC::Adds(
                scratch,
                scratch,
                static_cast<int32_t>(meta.missPrefixes[shard]) - 1,
                partWidth_);
            AscendC::PipeBarrier<PIPE_V>();
            ResolveSlotsVector(
                meta,
            ...
            // ResolveSlotsVector uses hitMask as scratch. Recreate the mask
            // from the immutable encoded value before selecting resident hits
            // versus newly assigned miss slots.
            AscendC::Maxs(
                scratch,
                encodedPrior,
                static_cast<int32_t>(0),
            ...
            AscendC::Select(
                scratch.ReinterpretCast<float>(),
                hitMask,
                encodedPrior.ReinterpretCast<float>(),
                rankOrResolved.ReinterpretCast<float>(),
                AscendC::SELMODE::VSEL_TENSOR_TENSOR_MODE,
                partWidth_);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Select(
                accumulated.ReinterpretCast<float>(),
                selectedMask,
                scratch.ReinterpretCast<float>(),
                accumulated.ReinterpretCast<float>(),
                AscendC::SELMODE::VSEL_TENSOR_TENSOR_MODE,
    ...
    AscendC::GlobalTensor<int64_t> targetSlots_;
    AscendC::TPipe pipe_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i32ABuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i32BBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i32CBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i32DBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i16ABuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i16BBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i16CBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> evictPoolBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> i64Buf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> maskABuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> maskBBuf_;
    AscendC::TBuf<AscendC::TPosition::VECCALC> blockTableBuf_;
    uint32_t requestCount_ = 0;
    uint32_t stateRowCount_ = 0;
    uint32_t dummyStateBase_ = 0;
    ...
    uint32_t blockTableEntries_ = 0;
    uint32_t partWidth_ = 0;
    uint32_t rolesPerRequest_ = 0;
};

class DSAResidentMissRankEncoderV2 {
public:
    __aicore__ inline void Init(
        __gm__ int32_t* shardCounts,
        __gm__ int16_t* priorSlots,
    ...
    __aicore__ inline void Process()
    {
        ...
        // The reused v1 union has just issued MTE3 writes for prior slots and
        // compact miss positions. Wait before applying scalar GM patches to
        // the same shard-private row.
        ...
        const uint32_t currentCount = static_cast<uint32_t>(
            ReadGlobalScalarFresh(
                shardCounts_, countOffset + kShardCurrentCount));
        const uint32_t missCount = static_cast<uint32_t>(
            ReadGlobalScalarFresh(
                shardCounts_, countOffset + kShardMissCount));
        const uint64_t shardOffset =
            (static_cast<uint64_t>(request) * shardCount_ + shard)
            * capacity_;
        if (missCount == 0) {
        ...
        auto prior = priorBuf_.Get<int16_t>();
        auto missPositions = missPositionBuf_.Get<int16_t>();
        CopyGlobalToLocalExact(
            prior, priorSlots_[shardOffset], currentCount);
        CopyGlobalToLocalExact(
            missPositions,
            shardMissPositions_[shardOffset],
            missCount);
        ...
        for (uint32_t miss = 0; miss < missCount; ++miss) {
            const uint32_t position = static_cast<uint32_t>(
                missPositions.GetValue(miss));
            prior.SetValue(
                position,
                static_cast<int16_t>(
                    -static_cast<int32_t>(miss) - 1));
        }
        Sync<AscendC::HardEvent::S_MTE3>();
        CopyLocalToGlobalExact(
            priorSlots_[shardOffset], prior, currentCount);
    }
...
extern "C" __global__ __aicore__ void
dsa_resident_sharded_union_v2_kernel(
    __gm__ int32_t* topkIndices,
    __gm__ int32_t* splitBoundary,
    __gm__ int32_t* rowReqIndices,
    __gm__ int32_t* shardPacked,
...
{
    {
        DSAResidentShardedUnionKernel unionOp;
        unionOp.Init(
            topkIndices, splitBoundary, rowReqIndices,
            shardPacked, shardMapping, shardCounts,
            requestStateIndices, requestStateGenerations,
    ...
    {
        DSAResidentMissRankEncoderV2 encoder;
        encoder.Init(
            shardCounts, priorSlots, shardMissPositions,
            requestCount, shardCount, shardCapacity,
            shardCountStride, shardCountRequestStride);
        encoder.Process();
    }
}
...
extern "C" __global__ __aicore__ void
dsa_resident_parallel_plan_v2_kernel(
    __gm__ int32_t* topkIndices,
    __gm__ int32_t* shardPacked,
    __gm__ int16_t* shardMapping,
    __gm__ int32_t* shardCounts,
    ...
    uint32_t blockSize)
{
    DSAResidentParallelPlanV2Kernel op;
    op.Init(
        topkIndices, shardPacked, shardMapping, shardCounts,
        requestBlockTable,
        requestStateIndices, requestStateGenerations,
        stateTokens, stateSlots, stateCounts, stateGenerations,
        priorSlots, shardMissTokens, shardEvictableSlots,
        missTokens, missCounts, targetSlots,
        requestCount, stateRowCount, dummyStateBase,
        rowsPerRequest, rowWidth, shardCount, capacity,
        shardCountStride, shardCountRequestStride, missCountStride,
        generationStride, blockTableWidth, blockSize);
    op.Process();
}
...
void dsa_resident_sharded_union_v2_impl(
    void* stream,
    void* topkIndices,
    void* splitBoundary,
    void* rowReqIndices,
...
void dsa_resident_parallel_plan_v2_impl(
    void* stream,
    void* topkIndices,
    void* shardPacked,
    void* shardMapping,
...
{
    (void)shardMissPositions;
    const uint32_t rolesPerRequest = 1 + 2 * shardCount;
    dsa_resident_parallel_plan_v2_kernel<<<
        requestCount * rolesPerRequest, nullptr, stream>>>(
        static_cast<int32_t*>(topkIndices),
        static_cast<int32_t*>(shardPacked),
        static_cast<int16_t*>(shardMapping),
        static_cast<int32_t*>(shardCounts),
